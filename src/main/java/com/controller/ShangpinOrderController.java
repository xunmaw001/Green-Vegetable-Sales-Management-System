    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(shangpinOrder !=null){
            ...
            BeanUtils.copyProperties( shangpinOrder , view );//把实体数据重构到view中
                ...
                //级联表
                PeisongEntity peisong = peisongService.selectById(shangpinOrder.getPeisongId());
                if(peisong != null){
                    BeanUtils.copyProperties( peisong , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setPeisongId(peisong.getId());
                }
                //级联表
...
//                            shangpinOrderEntity.setPeisongId(Integer.valueOf(data.get(0)));   //配送人员 要改的
...
//                            shangpinOrderEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R deliver(Integer id,Integer peisongId){
        logger.debug("refund:,,Controller:{},,ids:{}",this.getClass().getName(),id.toString());
        ShangpinOrderEntity  shangpinOrderEntity = new  ShangpinOrderEntity();;
        shangpinOrderEntity.setId(id);
        shangpinOrderEntity.setPeisongId(peisongId);
